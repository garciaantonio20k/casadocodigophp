...
function obtenerNombrePrioridad($codigo): string
{
    $prioridad = '';
    switch ($codigo) {
        case 1:
...
function convierteFechaFormularioParaMySQL($fechaFormulario): string
{
    if ($fechaFormulario === '') {
        return '';
    }
...
function convierteFechaMySQLParaFormulario($fechaMySQL): string
{
...
function mostrarPantallaFechaBD($fechaBD): string
{
...
function mostrarFinalizada($finalizada): string
{
    if ((int)$finalizada === 1) {
        return 'Si';
    }
